...
class installer{
	...
	private static function loadCSVSeeds($path, $dbi){
		...
		foreach($list as $rowIndex=>$line){
			...
			foreach($dbi->collection->getTables() as $index=>$targetTable){
				// Console::logln("[$index] ".$targetTable->name,"Green");
	...
	private static function importSeeds($seedObjects, $targetTable, $dbi){
		foreach($seedObjects as $tableName=>$seedObject){
			if($targetTable->name == $tableName){
				$seedObject = self::addDependingField($seedObject, $seedObjects, $targetTable, $dbi);
				$insertObj = self::parseSeedObjToInsertObj($seedObject);
				// foreach($insertObj->keys as $index=>$key){
				// 	Console::logln("[$index] $key	: ".$insertObj->values[$index], "LightGray", 6, true);
				// }
	...
	private static function addDependingField($seedObject, $seedObjects, $targetTable, $dbi){
		foreach($targetTable->dependencies as $dependency){
			...
			if(!isset($arr[$dependingTable->uid])){
				$where = self::parseSeedObjToWhere($seedObjects[$dependingTable->name], $dependingTable->uniques);
				$id = $dbi->getID($dependingTable->name, $where);
				$arr[$dependingTable->uid] = $id;
